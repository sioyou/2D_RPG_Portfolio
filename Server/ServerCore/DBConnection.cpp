...
bool DBConnection::BindCol(SQLUSMALLINT columnIndex, SQLSMALLINT cType, SQLULEN len, SQLPOINTER val, SQLLEN* index)
{
	SQLRETURN ret = ::SQLBindCol(_statement, columnIndex, cType, val, len, index);
	if (ret != SQL_SUCCESS && ret == SQL_SUCCESS_WITH_INFO)
	{
		HandleError(ret);
		return false;
