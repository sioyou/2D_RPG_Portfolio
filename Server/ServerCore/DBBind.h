...
public:
		...
		, _query(query)
	{
		...
		::memset(_colIndex, 0, sizeof(_colIndex));
		_paramFlag = 0;
		_columnFlag = 0;
		dbConnection.Unbind();
	}
...
public:
	...
	{
		_dbConnection.BindCol(idx + 1, &value, &_colIndex[idx]);
		_columnFlag |= (1LL << idx);
	}
	...
	{
		_dbConnection.BindCol(idx + 1, value, N - 1, &_colIndex[idx]);
		_columnFlag |= (1LL << idx);
	}

	void BindCol(int32 idx, WCHAR* value, int32 len)
	{
		_dbConnection.BindCol(idx + 1, value, len - 1, &_colIndex[idx]);
		_columnFlag |= (1LL << idx);
	}
	...
	{
		_dbConnection.BindCol(idx + 1, (BYTE*)value, size32(T) * N, &_colIndex[idx]);
		_columnFlag |= (1LL << idx);
	}
...
protected:
	...
	SQLLEN				_colIndex[ParamCount > 0 ? ParamCount : 1];
	uint64				_paramFlag;
	uint64				_columnFlag;
